...
// Data Structure: All 100 Projects
const projectsData = [
  // LEVEL 1 (1-25)
  {
    id: 1,
...
// Convert "Counter App" to "counter-app"
...
function renderProjects(projects) {
  if (projects.length === 0) {
    grid.innerHTML = `<div class="p-10 text-secondary col-span-full">No projects found matching your criteria.</div>`;
    return;
  }

  const allCardsHTML = projects
    .map((project) => {
      const cardColor = getPureRandomColor();
      const displayId = project.id.toString().padStart(2, "0");
      const folderName = createSlug(project.title);
      const linkHref = `./${folderName}/index.html`;

      return `
            <a href="${linkHref}" class="project-card p-8 md:p-10 flex flex-col group cursor-pointer relative overflow-hidden bg-background hover:bg-[#f6f3f2] transition-colors duration-200 block text-current no-underline w-full">
                <div class="flex justify-between items-start mb-12 md:mb-16">
                    <span class="font-label-xs text-[32px] md:text-[40px] font-extrabold text-outline-variant/30 group-hover:text-primary transition-colors duration-500">${displayId}</span>
                </div>
                
                <h3 class="font-headline-md text-headline-md mb-2 group-hover:translate-x-2 transition-transform duration-300 pr-4">${project.title}</h3>
                
                <p class="text-sm text-secondary font-code-sm mb-8 line-clamp-2">${project.desc}</p>
                
                <div class="mt-auto flex items-center justify-between">
                    <span class="font-label-xs text-xs uppercase tracking-widest text-secondary">LEVEL ${project.level}</span>
                    <div class="w-6 h-6 rounded-full transition-transform group-hover:scale-110 duration-300" style="background-color: ${cardColor};"></div>
                </div>
            </a>
            `;
    })
...
function processAndRender() {
  ...
  if (currentFilter !== "all") {
    const levelTarget = parseInt(currentFilter);
    processed = processed.filter((p) => p.level === levelTarget);
  }
